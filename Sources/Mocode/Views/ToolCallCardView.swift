...
struct SystemMessageAccentStripModifier: ViewModifier {
    let color: Color
...
extension View {
    func systemMessageAccentStrip(
        _ color: Color,
        cornerRadius: CGFloat = 12,
        width: CGFloat = 3
    ) -> some View {
        modifier(SystemMessageAccentStripModifier(color: color, cornerRadius: cornerRadius, width: width))
    }
}
...
struct ToolCallCardView: View {
    ...
    var body: some View {
            ...
            HStack(spacing: 8) {
                ...
                if let duration = model.duration, !duration.isEmpty {
                    Text(duration)
                        ...
                        .padding(.vertical, 3)
                        .background(MocodeTheme.surfaceLight.opacity(0.7))
                        .clipShape(Capsule())
                }
        ...
        .padding(.vertical, 10)
        .glassEffect(.regular.tint(kindAccent.opacity(0.12)), in: .rect(cornerRadius: 12))
        .systemMessageAccentStrip(kindAccent)
    ...
    private var statusChipBackground: Color {
        ...
        case .unknown:
            return MocodeTheme.surfaceLight.opacity(0.7)
        }
    }
    ...
    private var statusChipText: Color {
        ...
        case .unknown:
            return MocodeTheme.textSecondary
        }
    }
        ...
        case .kv(let label, let entries):
            if !entries.isEmpty {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    .padding(8)
                    .background(MocodeTheme.surface.opacity(0.6))
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        ForEach(identifiedItems) { item in
                            HStack(alignment: .top, spacing: 8) {
                                Circle()
                                    .fill(item.index == identifiedItems.count - 1 ? kindAccent : MocodeTheme.textMuted)
